...
module MakePolyExtendedField (P : POLY_EXTENDED_FIELD_PARAM) :
  POLY_EXTENDED_FIELD with module Ring = P.Ring = struct
  module Ring = P.Ring
  ...
  let inv a =
    if a = zero then raise Division_by_zero
      ...
      if not (deg g = 0) then
        failwith "No inverse (not a field: p is not irreductible)"
...
module rec MakePoly : functor (F : FIELD) ->
  POLY_EUCLIDEAN_RING with module F = F =
functor
  (F : FIELD)
  ->
  struct
    ...
    let constant_coeff (p : t) =
      let p = normalize p in
      if p = zero then F.zero else p.(0)
    ...
    let euclidean_div (a : t) (b : t) : t * t =
      ...
      let degb = deg b in
      if degb < 0 then failwith "Don't divide polynomes by 0"
      else
        let q = ref zero in
        let r = ref a in
        while deg !r >= degb do
          let fact =
            F.div (leading_coeff !r) (leading_coeff b) *. (x **^ (deg !r - degb))
          in
          r := !r -^ (fact *^ b);
          q := !q +^ fact
        done;
        (normalize !q, normalize !r)
    (* I still don't fucking know why it needs to be normalized again here *)
    ...
    let rec egcd a b =
      if b = zero then (a, one, zero)
        ...
        let inv_g_coef = F.inv @@ leading_coeff g in
        (* We want the gcd to be unitary *)
    ...
    let to_int p =
      if F.order = -1 then F.to_int (constant_coeff p)
      else
        let q = F.order in
        Array.fold_right (fun coeff acc -> F.to_int coeff + (acc * q)) p 0

    let of_int a =
      if F.order = -1 then [| F.of_int a |]
      else
        let q = F.order in
    ...
    let rec cyclotomic (n : int) : t =
      if n = 1 then x -^ one
        ...
        let preds : t =
          List.fold_left
            (Fun.flip @@ Fun.compose mul cyclotomic)
            (cyclotomic 1) (Utils.divisors n)
        in
