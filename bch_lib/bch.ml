...
module type BCH_PARAM = sig
  ...
  val primitive_p : FqX.t
  (* An irreductible factor of phi_{q^m-1}, "polynome primitif de degré m" *)
...
module type Code = sig
  ...
  val correct : int array -> int array option
end
...
module BchCode (P : BCH_PARAM) = struct
  ...
  let primitive_p = P.primitive_p
  let m = FqX.deg primitive_p
  ...
  module ZnZ = Fields.MakeExtendedField (struct
    (* Not a field but I swear I don't use inv *)
  ...
  let g i : FqX.t =
    let open FqmX in
    Array.map Fqm.constant_coeff
    @@ IntSet.fold
         (fun l acc -> acc *^ (x -^ ((alpha *. one) **^ l)))
         (sigma i) one

  let full_sigma =
    List.fold_left
      (fun acc i -> IntSet.union acc (sigma i))
      IntSet.empty
      (List.init (delta - 1) (( + ) 1))
  ...
  let encode_mul a =
    let open FqX in
    if deg a >= k then failwith "Only accept messages of length k"
    else a *^ full_g

  let decode_mul ag =
    let open FqX in
    if deg ag >= n then failwith "Only decode messages of length n"
    else
  ...
  let decode_sys p : FqX.t =
    let open FqX in
    if deg p >= n then failwith "Only decode messages of length n"
    else Array.sub p (n - k) k

  let encode =
    let open FqX in
    let ( @. ) = Fun.compose in
    to_array @. encode_sys @. of_array

  let decode =
    let open FqX in
    let ( @. ) = Fun.compose in
    to_array @. decode_sys @. of_array
  ...
  let alpha_powers =
    ...
    a
  (* Basically contains every element of the field... Maybe use the Chien Search (it mays not be worth it, we're not on a hardware plan) *)

  let sub_alpha_powers = Array.sub alpha_powers 1 (2 * t)

  (* Forney and Sugiyama algorithm *)
  let correct_p (r' : FqX.t) : (FqX.t, FqmX.t) result =
    let open FqmX in
    ...
    if coef = Fqm.zero then Result.error bi
    else
      ...
      Array.iteri
        (fun i alphap ->
          ...
          if eval sigma alphinv = Fqm.zero then
            e.(i) <-
              Fqm.sub Fqm.zero
              @@ Fqm.div (eval omega alphinv) (eval sigma' alphinv))
        alpha_powers;
      Result.ok @@ Array.map Fqm.constant_coeff (r' -^ e)

  let correct a =
    let open FqX in
    match correct_p (of_array a) with
    | Result.Ok m -> Some (to_array m)
    | Result.Error _ -> None
end
